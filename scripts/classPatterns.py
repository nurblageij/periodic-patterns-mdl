import numpy, re, sys, copy, itertools
import pdb
...
def makeOccsAndFreqsMedian(tmpOccs):
    ...
    nbOccs[-1] = numpy.sum(nbOccs.values())*1.
    
    symOccs = dict(tmpOccs.items())
    med = numpy.median(tmpOccs.values())
    symOccs["("] = med
    symOccs[")"] = med
    symSum = numpy.sum(symOccs.values())*1.
    adjFreqs = {}
    orgFreqs = {}
...
def key_to_l(key):
    try:
        return [map(int, b.split(SUB_SEP)) for b in key.split(SUP_SEP)]
    except:
        return []
def l_to_key(l):
...
class DataSequence(object):
    ...
    def __init__(self, seq):
        ...
        t_end = numpy.max(evEnds.values())
        t_start = numpy.min(evStarts.values())
...
class Pattern(object):
    ...
    def getNidFromKey(self, k):
        ...
        if type(k) is list:    
            ...
            key_ints = key_to_l(k)
        if len(key_ints) == 0: return None ### something went wrong
    ...
    def codeLength(self, t0, E, data_details, match=None, nid=0):
        ...
        if self.hasNestedPDs():
            ...
            if self.transmit_Tmax:
                ...
                Tmax_rep_val = numpy.max(tmpd.values()) - tmpd[o_za]
    ...
    def getEvOccs(self):
        if self.isPattern():
            if self.ev_occ is None:
                self.ev_occ = zip(*[self.O, self.getEventsMinor(rep=True)])
            return self.ev_occ
        else:
...
class CandidatePool(object):
    ...
    def getCidsForMinorK(self, mK):
        return self.map_minorKs.get(mK, {}).values()
