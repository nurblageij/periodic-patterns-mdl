...
COLORS_CATS_RED = ["#FFFAD2", "#FFE3AA", "#F9BD7E", "#ED875E", "#D24D3E", "#AE1C3E"]
COLORS_CATS_GREY = ["#"+6*("%s" % s) for s in range(1,10)]+ ["#"+6*chr(s) for s in range(ord("A"),ord("F"))]
COLORS_CATS_OTHER = ["#7FB972", "#4EB265", "#117755"]

COLORS_CATS = {}
COLORS_CATS.update(dict([(k,COLORS_CATS_GREY[j]) for (j,k) in enumerate([i for i, l in enumerate(CMP_OUT_CODES) if re.search("same", l)])]))
COLORS_CATS.update(dict([(k,COLORS_CATS_RED[j]) for (j,k) in enumerate([i for i, l in enumerate(CMP_OUT_CODES) if re.search("worse", l)])]))
COLORS_CATS.update(dict([(k,COLORS_CATS_BLUE[j]) for (j,k) in enumerate([i for i, l in enumerate(CMP_OUT_CODES) if re.search("better", l)])]))
COLORS_CATS.update(dict([(k,COLORS_CATS_BLUE[j]) for (j,k) in enumerate([i for i, l in enumerate(CMP_OUT_CODES) if re.search("better", l)])]))
COLORS_CATS.update(dict([(k,COLORS_CATS_OTHER[j]) for (j,k) in enumerate(sorted(set(range(len(CMP_OUT_CODES))).difference(COLORS_CATS.keys())))]))
...
FIELDS_COMB = ['t0_low', "seriesL"] #, 't0_up', 'k_low', 'k_up', "seriesL", "seriesX"]
...
def rep_collect_results(series_basis, xps_rep):
    ...
    for fi, fn in enumerate(glob.glob(match_summaries)):
        tmp = re.search("_(?P<seriesL>[UVWS])(comb)?(?P<series_id>[0-9\-]+)_summary\.txt", fn)        
        if tmp is not None:
            cl_pairs = []
            patts = {"H": [], "F": []}
            ps = {"H": set(), "F":set()}
                ...
                for line in fp:
                    ...
                    if ttmp is not None:
                        if len(cl_pairs) == 0:
                            ...
                            cl_pairs.extend(map(float, [ttmp.group("cl"), ttmp.group("cl_res")]))
                            which = "F"
                    else:
            ...
            if k not in collected_results:
                ...
                collected_results[k] = {"params": params, "counts": cat_counts, "cl_pairs": [], "details": []}
            collected_results[k]["counts"][cat] += 1
            collected_results[k]["cl_pairs"].append(cl_pairs)
            collected_results[k]["details"].append({"patts": patts, "ps": ps, "series_id": series_id, "seriesL": seriesL})
...
def collect_results(series_basis):
    collected_results = []
    match_summaries = "%s%s[0-9\-]*_series-summary.txt" % (XPS_REP, series_basis)
    for fi, fn in enumerate(glob.glob(match_summaries)):
        cl_pairs = []
        tot = -1
        cat_counts = [0 for i in range(len(CMP_OUT_CODES))]
        with open(fn) as fp:
            for li, line in enumerate(fp):
                if li == 1:
                    ...
                    tmp = re.match("(?P<nb_cat>[0-9]*)/(?P<nb_tot>[0-9]*)\t\([0-9\.]+\)\t(?P<cat_id>[0-9]*) <<", line)
                    if tmp is not None:
                        cat_counts[int(tmp.group("cat_id"))] = int(tmp.group("nb_cat"))
                         
                    elif re.match("^[0-9\. ]*$", line):
                        cl_pairs.append(map(float, line.strip().split()))
        ...
        collected_results.append({"params": params, "counts": cat_counts, "cl_pairs": numpy.array(cl_pairs)})
...
def draw_synthe_boxes(series_basis):
    ...
    vs_all = [cs["cl_pairs"][:,1]/cs["cl_pairs"][:,2] - cs["cl_pairs"][:,0]/cs["cl_pairs"][:,2] for cs in collected_results]
    ...
    for li, level in enumerate(TLVLS):
        ...
        for bi, (inner, p_down) in enumerate(blocks):
            ...
            rids = [(ci, (p["params"]["noise_lvl"], p["params"]["noise_dens"]), "%s" % (p["params"]["noise_dens"]))  for ci, p in enumerate(collected_results) if (p["params"]["inner"] == inner and p["params"]["p_down"] == p_down) and (p["params"]["level"] == level)]
            ...
            for ni, noise_lvl in enumerate(NLVLS):
                srids = [(ssi, sri[0]) for ssi, sri in enumerate(rids) if  sri[1][0] == noise_lvl]
                if len(srids) > 0:
                    vs = [vs_all[r[1]] for r in srids]
                    pps = [pos[r[0]] for r in srids]
                    box = grid[li].boxplot(vs, vert=False, positions=pps, widths=.8, patch_artist=True, **all_box_props)
                    for b in box["boxes"]:
            ...
            if li == 0:
                ytick_lbls_all.extend(["%s % 4d/%d" % (r[-1], collected_results[r[0]]["counts"][0], numpy.sum(collected_results[r[0]]["counts"])) for r in rids])
            else:
                ytick_lbls_all.extend(["%d/%d" % (collected_results[r[0]]["counts"][0], numpy.sum(collected_results[r[0]]["counts"])) for r in rids])
        ...
        grid[li].plot([PERFECT_VAL, PERFECT_VAL], [ymin, ymax], ':', color="#999999", zorder=-1)
        ...
        for si in range(len(suboffs)):
            grid[li].text(x_min+(x_max-x_min)/2., suboffs[si], sublbls[si], horizontalalignment='center', fontsize=14)
            grid[li].plot([x_min+.1, x_max-.1], [suboffs[si]+.35, suboffs[si]+.35], "k", linewidth=0.1)

    plt.subplots_adjust(bottom=0.2)    
    lls = [plt.plot(-10, -10, COLOR_NLVL[ni], linewidth=10)[0] for ni, noise_lvl in enumerate(NLVLS)]
    labels= ["%d" % noise_lvl for ni, noise_lvl in enumerate(NLVLS)]
    labels[0] = "shift noise level:     "+labels[0] 
    
    bb = (fig.subplotpars.left, fig.subplotpars.bottom-0.12, fig.subplotpars.right-fig.subplotpars.left,.1)
...
def draw_synthe_boxes_combs(series_basis):
    ...
    for li, sxx in enumerate([0]):
        ...
        for bi, sss in enumerate(blocks):
            ...
            rids = [(ci, (p["params"]["t0_low"], p["params"]["t0_up"]), lgd.get(p["params"]["t0_low"], "-"))  for ci, p in enumerate(collected_results) if (p["params"]["seriesL"] == sss)]
            rids.sort(key=lambda x: x[1])
            ...
            for ni, noise_lvl in enumerate([(1.2, 1.5), (.2,.9)]):
                srids = [(ssi, sri[0]) for ssi, sri in enumerate(rids) if  sri[1] == noise_lvl]
                if len(srids) > 0:
                    vs = [vs_all[r[1]] for r in srids]
                    pps = [pos[r[0]] for r in srids]
                    box = grid[li].boxplot(vs, vert=False, positions=pps, widths=.8, patch_artist=True, **all_box_props)
        ...
        for si in range(len(suboffs)):
            grid[li].text(x_min+(x_max-x_min)/2., suboffs[si], sublbls[si], horizontalalignment='center')
            grid[li].plot([x_min+.1, x_max-.1], [suboffs[si]+.35, suboffs[si]+.35], "k", linewidth=0.1)
