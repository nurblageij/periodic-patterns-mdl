import numpy, re, sys, datetime, itertools, os, glob
import pickle
...
from readData import readSequence, readSequenceSacha
...
def prepare_candidate_nested(cp_det, P_minor, cmplx_candidates):
    ...
    pr_key = cp_det[1]
    tmp = pr_key.split("_")
    prds = map(int, tmp[0].split("+"))
    lens = map(int, tmp[1].split("*"))[::-1]
...
def filter_candidates_cover_slow(cands, dcosts, min_cov=1, adjust_occs=False, cis=None):
    ...
    selected = []
    covered = set()
    max_eff = numpy.max(dcosts.values())
    while len(cis) > 0:
        nxti = cis.pop(0)
...
def filter_candidates_cover(cands, dcosts, min_cov=1, adjust_occs=False, cis=None):
    ...
    selected = []
    covered = set()
    max_eff = numpy.min(dcosts.values())
    while len(cis) > 0:
        nxti = cis.pop(0)
        if cands[nxti].getCostUncoveredRatio() <= max_eff:
...
def mine_seqs(seqs, fn_basis="-", max_p=None, writePCout_fun=None):
    ...
    if PICKLE == -1 and fn_basis is not None and fn_basis != "-":
        ...
        fp = open(fpick, "w"); pickle.dump(cpool, fp); fp.close()
...
if __name__ == "__main__":
    ...
    series_params = {}
    series_params["bugzilla_0_rel_all"] = {"filename": DATA_REP+"traces/trace_bugzilla_0_data.dat",
                                        "timestamp": False, "events": ["*"]}
    series_params["bugzilla_1_rel_all"] = {"filename": DATA_REP+"traces/trace_bugzilla_1_data.dat",
                                       "timestamp": False, "events": ["*"]}
    series_params["3zap_0_rel"] = {"filename": DATA_REP+"traces/trace_kptrace_3zap_0_data.dat",
                                   "timestamp": False, "events": ["*"]}
    series_params["3zap_1_rel"] = {"filename": DATA_REP+"traces/trace_kptrace_3zap_1_data.dat",
                                   "timestamp": False, "events": ["*"]}

    series_params["samba_auth_abs"] = {"filename": DATA_REP+"samba_commits/commit_auth_data.dat",
                                  "timestamp": True, "events": ["*"]}

    for grain in [1, 15, 30, 60, 720, 1440]:
        series_params["sacha_18_absI_G%d" % grain] = {"filename": DATA_REP+"sacha/data_18-03-22_lagg200NL.txt",
                                                      "timestamp": True, "events": ["*"], "granularity": grain, "I": True}

    series_params["sacha_18_rel"] = {"filename": DATA_REP+"sacha/data_18-03-22_lagg200NL.txt",
                                     "timestamp": False, "events": ["*"]}
    # series_params["sacha_18_rel_2000"] = {"filename": DATA_REP+"sacha/data_18-03-22_lagg200NL.txt",
    #                                  "timestamp": False, "events": ["*"], "drop_event_codes":[0, 126, 33]}
    # series_params["sacha_18_abs_2000W"] = {"filename": DATA_REP+"sacha/data_18-03-22_lagg200NL.txt",
    #                                  "timestamp": True, "events": ["*"], "max_len": 2000, "max_p": 7*24*60}    
    
    for f in glob.glob(DATA_REP+"UbiqLog/prepared/*_data.dat"):
        bb = f.split("/")[-1].strip("_data.dat")
        if not re.search("ISE", bb):
            series_params["UbiqLog_%s_rel" % bb] = {"filename": f, "timestamp": False, "events": ["*"]}
        else:
            series_params["UbiqLog_%s_abs" % bb] = {"filename": f, "timestamp": True, "events": ["*"]}
    ...
    for series in lseries:
        if series in series_params:

            if re.match("sacha", series):
                seqs = readSequenceSacha(series_params[series])
            else:
                seqs = readSequence(series_params[series])
                
            if run_id == "display":
                print "DISPLAY %s" % series
                disp_seqs(seqs)
            else:
                fn_basis = "%s%s_%s" % (XPS_REP, series, run_id)                
                print "RUNNING %s" % series                
                mine_seqs(seqs, fn_basis, max_p=series_params[series].get("max_p"))
        else:
            print "Series %s does not exist! %s" % (series, series_params.keys())
