    ...
    with open(fn) as fp:
        for line in fp:
            if re.match("t0\=", line):
                ...
                if is_abs:
                    pp = parts[0].split("=")
                    parts[0] = "t0=%s" % (abs_init_T+datetime.timedelta(minutes=grain*int(pp[1])))

                Ptree = parts[1]
                for p in [pp for pp in re.finditer("([^() ]+)", parts[1])][::-1]:
                    if Ptree[p.start():p.end()] in codes:
                        Ptree = "%s%s%s" % (Ptree[:p.start()], codes[Ptree[p.start():p.end()]], Ptree[p.end():])
                    elif is_abs:
                        tmp = re.match("(?P<bef>\[?[dp]\=)(?P<time>[0-9]+)\]", Ptree[p.start():p.end()])
                        if tmp is not None and int(tmp.group("time")) != 0:
                            ...
                            td = "%s" % datetime.timedelta(minutes=grain*int(tmp.group("time")))
                            tt = re.sub(":00$", "", re.sub(" days?, ", "D,", td))
                            Ptree = "%s%s%s]%s" % (Ptree[:p.start()], tmp.group("bef"), tt, Ptree[p.end():])
                parts[1] = Ptree
