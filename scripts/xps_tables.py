...
INTER_LBL = {"S": "$\\collS$", "V": "$\\collV$", "H": "$\\collH$", "V+H": "$\\collVH$", "F": "$\\collF$"}
...
def get_agg(Xorg, rids, inter, ifrmt, ifield, map_field_num, ops):
    ...
    if kk in map_field_num:
        ...
        if re.match("frac_", ifield):
            ...
            if ifield == "frac_patt_cl":
                num, den = ("patt_cl", "compressed_cl")
            vs = [get_frac(Xorg, rid, inter, map_field_num, num, den) for rid in rids]
    if len(vs) > 0:
        if len(ops) == 1:
...
Xorg = numpy.hstack([Xorg, numpy.vstack([(Xorg[:,map_field_num["S_prc_cl"]] - Xorg[:,map_field_num["%s_prc_cl" % ii]]) for ii in inters]).T])


######## DATA STATS
DATASET_FIELDS = [("%d", "size_O", "$\len{\seq}$"), ("%d", "size_T", "$\\tspan{\seq}$"), ("%d", "size_ABC", "$\\abs{\\ABC}$"),
                  ("%d", "evNbO_median", "$\\len{\\seq[\\alpha]}$"), ("%d", "evNbO_max", "$\\len{\\seq[\\alpha]}$"),
...
DATA_HEAD = SEP.join([""]+['$\\len{\\seq}$', '$\\tspan{\\seq}$', '$\\abs{\\ABC}$', '\multicolumn{2}{c}{$\\len{\\seq[\\alpha]}$}',"",  '$\\clEmpty$', '\multicolumn{2}{c}{RT (s)}'])
DATA_HEAD_SEC = SEP.join(4*[""]+['$\\omed$', '$\\max$', '', '', 'cycles', 'overall'])
##########################
max_per_table = 7
indivs = []
rids = [r for r in range(len(rlabels)) if not re.match("UbiqLog", rlabels[r]) and not re.match("sacha", rlabels[r])]
indivs.append({"title": "application log trace", "rids": rids, "ref": "traces"})
...
for mtch, tit, ref in [("sacha.*", "\\dstSacha{}", "sacha"), ("UbiqLog.*ISE", "\\dstUbiAR{\\iAbs}", "UbiqLog-ISE-abs"), ("UbiqLog.*IS[_\-]rel", "\\dstUbiAR{\\iRel}", "UbiqLog-IS-rel")]:
    ...
    if ref == "sacha":
        skeys = dict([(rid, (int((rlabels[rid]+"G9999").split("G")[1]), "2000" not in rlabels[rid], "absI" not in rlabels[rid])) for rid in rids])
...
for x in ["X"]:
    ...
    str_table += DATA_HEAD + EoL + "\n"
    str_table += DATA_HEAD_SEC + EoL + "\n"
    str_table += ("\\cmidrule{2-%d}" % (len(DATASET_FIELDS)+1))+ EoL + "[-.5em]\n"
    
    data_rows = []
    # rrs = sorted(group["rids"], key=lambda x: Xorg[x, map_field_num["size_O"]])
    rrs = [0]
    for gi, group in enumerate(indivs):
        dname = "%s (%d)" % (group["title"], len(group["rids"])) # re.sub("-ISE?-[a-z]+$", "", re.sub("^UbiqLog-", "", rlabels[rid]))
        str_table += ("\\multicolumn{%d}{c}{%s}" % (len(DATASET_FIELDS)+1, dname)) + EoL +"\n"
        str_table += "\\midrule\n"
        for op in ops:
            data_rows.append([op[0]])        
            for (ifrmt, ifield, h) in DATASET_FIELDS:
                if ifield is not None:
                    data_rows[-1].append(get_agg(Xorg, group["rids"], None, ifrmt, ifield, map_field_num, [op]))
                    # data_rows[-1].append(("$"+ifrmt+"$") % Xorg[rid, map_field_num[ifield]])
                else:
                    data_rows[-1].append("")
            str_table += SEP.join(data_rows[-1]) + EoL + "\n"
...
WIDE_FIELDS = [("%.2f", "prc_cl", "$\\prcCl{}$"), ("%d", "compressed_cl", "$\\clCC$"),
                ...
                ("%d", "nb_residuals", "$\\abs{\\resSet}$"), ("%d", "nb_patts", "$\\abs{\\ccycle}$"),
                ("%d", "nb_simple", "$\\nbS$"), ("%d", "nb_nested", "$\\nbV$"),
                ("%d", "nb_concat", "$\\nbH$"), ("%d", "nb_other", "$\\nbTD$"),
                ("%d", "frac_nbO_>3", "$\\nbOTC$"), ("%d", "nbO_median", "$\\nbOmed$"), ("%d", "nbO_max", "$\\nbOmax$")] #,
                ## ("%d", "r_median", "$r^m$"), ("%d", "r_max", "$r^+$")] ("%s", "split_nbs", "$\\#$"), ("%d", "nbO_mean", "$o^\sim$"),
nbC = len(WIDE_FIELDS)
WIDE_COL_FRMT = "@{}c@{\\hspace{3ex}}"+2*"r@{\\hspace{1ex}}"+"r@{\\hspace{3ex}}"+2*"r@{\\hspace{1ex}}"+ 3*"c@{\\,/\\,}"+"c@{\\hspace{3ex}}"+ 2*"r@{\\hspace{1ex}}"+"r@{}"
...
tmp += ["\\parbox[b][1em][b]{.5cm}{\\centering %s}" % df[-1] for df in WIDE_FIELDS[nbC-7:-3]]
...
indivs = []
rids = [r for r in range(len(rlabels)) if not re.match("UbiqLog", rlabels[r]) and not re.match("sacha", rlabels[r])]
indivs.append({"title": "application log trace sequences", "rids": rids, "ref": "traces"})
...
for group in indivs:
    ...
    str_table += ("\\cmidrule{2-%d}" % (len(WIDE_FIELDS)+1))+ EoL + "[-.5em]\n"
        
    xps_rows = []
    rrs = group["rids"] #sorted(group["rids"], key=lambda x: Xorg[x, map_field_num["size_O"]])
    for rr, rid in enumerate(rrs):
        dname = getDName(rlabels[rid])
        str_table += ("\\multicolumn{%d}{c}{%s}" % (len(WIDE_FIELDS)+1, dname)) + EoL +"\n"
        str_table += "\\midrule\n"
        xps_rows.append([])
        cmp_vals = [Xorg[rid, map_field_num["%s_%s" % (inter, CMP_FIELD)]] for inter in inters]
        ...
        for ii, inter in enumerate(interss):
            ...
            for (ifrmt, ifield, h) in WIDE_FIELDS:
                xps_rows[-1][-1].append(get_entry(Xorg, rid, inter, ifrmt, ifield, map_field_num, ids_min))
...
SHORT_COL_FRMT = "@{}c@{\\hspace{1ex}}r@{\\hspace{.7ex}}r@{\\hspace{.7ex}}"+ 3*"c@{/}"+"c@{\\hspace{.4ex}}r@{}"
...
tmp += ["\\parbox[b][1em][b]{.2cm}{\\centering %s}" % df[-1] for df in SHORT_FIELDS[2:-1]]
tmp += ["%s" % df[-1] for df in SHORT_FIELDS[-1:]]
...
indivs.append({"title": "\\dstSacha{} and \\dstSamba{}", "rids": rids, "ref": "sa"})
...
for ggi, group in enumerate(indivs):
    ...
    str_table += ("\\cmidrule{2-%d}" % (len(SHORT_FIELDS)+1))+ EoL + "[-.5em]\n"
    ...
    for rr, rid in enumerate(rrs):
        ...
        if True: #re.match("3zap", rlabels[rid]):
            interss = inters
        dname = getDName(rlabels[rid])
        str_table += ("\\multicolumn{%d}{c}{%s}" % (len(SHORT_FIELDS)+1, dname)) + EoL +"\n"
        str_table += "\\midrule\n"
        xps_rows.append([])
        cmp_vals = [Xorg[rid, map_field_num["%s_%s" % (inter, CMP_FIELD)]] for inter in inters]
        min_val = numpy.min(cmp_vals)
...
for x in ["X"]:
    ...
    xps_rows = []
    rrs = [0] #group["rids"] #sorted(group["rids"], key=lambda x: Xorg[x, map_field_num["size_O"]])
    for gi, group in enumerate(indivs):
        dname = "%s (%d)" % (group["title"], len(group["rids"])) # re.sub("-ISE?-[a-z]+$", "", re.sub("^UbiqLog-", "", rlabels[rid]))
        str_table += ("\\multicolumn{%d}{c}{%s}" % (len(SHORT_FIELDS)+1, dname)) + EoL +"\n"
        str_table += "\\midrule\n"
        ...
        for ii, inter in enumerate(inters):
            xps_rows[-1].append([INTER_LBL[inter]])        
            for (ifrmt, ifield, h) in SHORT_FIELDS:
                xps_rows[-1][-1].append(get_agg(Xorg, group["rids"], inter, ifrmt, ifield, map_field_num, ops))
            str_table += SEP.join(xps_rows[-1][-1]) + EoL + "\n"
        if gi < len(indivs)-1:
            str_table += "[.5em]\n"
    str_table += "\\bottomrule\n"
